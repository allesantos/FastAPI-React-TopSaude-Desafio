...
const CustomerList: React.FC = () => {
  ...
  const fetchCustomers = async () => {
    ...
    // Nota: Backend ainda não suporta filtros de nome/email/documento
    // na listagem. Por enquanto, apenas paginação funciona.
    // TODO: Implementar filtros no backend se necessário

    const result = await execute(() => customerService.listCustomers(params));
    
    if (result) {
      // Filtro local enquanto backend não suporta
      ...
      if (debouncedEmail) {
        filtered = filtered.filter((c) =>
          c.email.toLowerCase().includes(debouncedEmail.toLowerCase())
        );
      }

      if (debouncedDocument) {
        const cleanDoc = debouncedDocument.replace(/\D/g, '');
        filtered = filtered.filter((c) =>
          c.document.replace(/\D/g, '').includes(cleanDoc)
        );
      }
